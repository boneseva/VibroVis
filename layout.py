...
def create_layout(df):
    ...
    *** UPDATED *** to set smart defaults for dropdowns to prevent "No data" on load.
    ...
    return html.Div([
        html.Div([
            html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        html.Button("📋 Table", id="view-toggle-btn", n_clicks=0, className="app-button", 
                                   style={'marginLeft': '10px', 'fontSize': '0.9em'}),
                    ], style={'display': 'flex', 'alignItems': 'center', 'marginBottom': '10px'}),
                ], style={'display': 'flex', 'alignItems': 'center', 'marginBottom': '10px'}),
                
                html.Div(id='main-view-container', children=[
                    html.Div(style={'position': 'relative', 'flexGrow': '1', 'minHeight': '0', 'width': '100%', 'height': '100%'}, children=[
                        dcc.Graph(id='scatter')
                    ]),
                    html.Div(id='table-view-container', style={'display': 'none', 'flex': '1', 'minHeight': '0', 'flexDirection': 'column', 'height': '100%'}, children=[
                        html.Div([
                            html.Label('Visible Columns:', style={'marginRight': '10px', 'fontWeight': 'bold', 'color': '#2a3f5f'}),
                            dcc.Dropdown(
                                id='table-column-selector',
                                options=[
                                    {'label': 'Date/Time', 'value': 'date_time'},
                                    {'label': 'Start', 'value': 'timestamp_start'},
                                    {'label': 'End', 'value': 'timestamp_end'},
                                    {'label': 'Duration (s)', 'value': 'duration'},
                                    {'label': 'Cluster', 'value': 'cluster_id'},
                                    {'label': 'File Name', 'value': 'file_name'},
                                    {'label': 'Count', 'value': 'clip_count'},
                                    {'label': 'Location', 'value': 'location'},
                                    {'label': 'Microlocation', 'value': 'microlocation'},
                                    {'label': 'Channel', 'value': 'channel'},
                                    {'label': 'Recorder Type', 'value': 'recorder_type'},
                                    {'label': 'X Coordinate', 'value': 'x'},
                                    {'label': 'Y Coordinate', 'value': 'y'},
                                    {'label': 'Temperature (°C)', 'value': 'temperature'},
                                    {'label': 'Hour', 'value': 'hour'},
                                ],
                                value=['date_time', 'timestamp_start', 'timestamp_end', 'cluster_id', 'file_name', 'duration'],
                                multi=True,
                                clearable=False,
                                style={'minWidth': '300px', 'flex': '1'}
                            )
                        ], style={'display': 'flex', 'alignItems': 'center', 'padding': '10px', 'flexShrink': '0', 'borderBottom': '1px solid #ddd', 'backgroundColor': '#f9f9f9'}),
                        html.Div(id='data-table-wrapper', style={'flex': '1', 'minHeight': '0', 'display': 'flex', 'flexDirection': 'column', 'position': 'relative', 'height': '100%'}, children=[
                            dash_table.DataTable(
                                ...
                                sort_mode='multi',
                                filter_action='none',
                                sort_by=[{'column_id': 'date_time', 'direction': 'asc'}],
                                ...
                                style_cell_conditional=[
                                    {'if': {'column_id': 'play_button'}, 'width': '20px', 'textAlign': 'center', 'cursor': 'pointer'},
                                    {'if': {'column_id': 'file_name'}, 'minWidth': '200px'},
                                    {'if': {'column_id': 'date_time'}, 'minWidth': '150px'},
                                ],
                                style_header={
                        ...
                        ], style={'flexShrink': '0', 'marginTop': '10px', 'padding': '0px'}),
                    ])
                ]),
        ...
        dcc.Store(id='view-mode-store', data='scatter'),
        dcc.Store(id='table-data-store'),
        dcc.Store(id='table-column-visibility-store', data=['date_time', 'timestamp_start', 'timestamp_end', 'cluster_id', 'file_name', 'duration']),
        dcc.Store(id='table-full-data-store'),
        dcc.Store(id='table-style-store'),
    ], id='main-container')