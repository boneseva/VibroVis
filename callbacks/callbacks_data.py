    ...
    def load_data_into_server_cache(selected_model, selected_location):
        if not selected_model or not selected_location:
            ...
            if not os.path.exists(data_path):
                ...
                try:
                    # Try loading with temperature first
                    cols_with_temp = cols_to_load + ['temperature']
                    model_df = pd.read_parquet(
                        ...
                        columns=cols_with_temp
                    )
                except (KeyError, ValueError, Exception) as e:
                    # If temperature column doesn't exist, load without it
                    if 'temperature' in str(e).lower() or 'No match for FieldRef' in str(e):
                        print("Temperature column not found in parquet file, will generate it...")
                        model_df = pd.read_parquet(
                            data_path,
                            filters=[
                                [('model_name', '==', selected_model),
                    ...
                    if 'temperature' not in model_df.columns:
                        # Base temperature varies by month (seasonal effect)
                        month = pd.to_datetime(model_df['day_dt']).dt.month
                        seasonal_base = 15 + 10 * np.sin((month - 3) * np.pi / 6)  # Range roughly 5-25°C
                        
                        # Diurnal variation: colder at night, warmer during day
                        hour = pd.to_datetime(model_df['time_of_day']).dt.hour
                        diurnal_variation = 8 * np.sin((hour - 6) * np.pi / 12)  # Peak around 2 PM, minimum around 2 AM
                        
                        # Add some random variation (±2°C)
                        np.random.seed(42)  # For reproducibility
                        random_variation = np.random.normal(0, 1.5, len(model_df))
                        
                        # Combine all effects
                        model_df['temperature'] = seasonal_base + diurnal_variation + random_variation
                        # Clip to reasonable range (0-35°C)
                        model_df['temperature'] = model_df['temperature'].clip(0, 35)
                        model_df['temperature'] = model_df['temperature'].round(1)
