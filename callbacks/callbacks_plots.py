...
def create_dark_color(base_hex):
    """Create a darker version of the base color for the temperature line."""
    r, g, b = hex_to_rgb(base_hex)
    # Darken by scaling towards black (0, 0, 0)
    # Mix 80% original + 20% black for darker effect
    r = int(r * 0.8)
    g = int(g * 0.8)
    b = int(b * 0.8)
...
def create_pale_color(base_hex, alpha=0.2):
    """Create a pale version of the base color for the temperature band."""
    r, g, b = hex_to_rgb(base_hex)
    # Blend with white (60% white, 40% original) for less pale, more visible effect
    r = int(r * 0.4 + 255 * 0.6)
    g = int(g * 0.4 + 255 * 0.6)
    b = int(b * 0.4 + 255 * 0.6)
    # Ensure values stay in valid range
...
def register_plot_callbacks(app):
    ...
    def update_figure(model_ready_signal, selected_channels, selected_num_clusters,
        ...
        if has_checkbox_inputs and not should_force_defaults:
            # Always apply cluster filter if clusters are selected, regardless of what triggered the update
        ...
        # Create scatter plot - use go.Figure directly to avoid template issues
        fig = go.Figure()
        for cluster_id_str in dff['cluster_id_str'].unique():
            cluster_data = dff[dff['cluster_id_str'] == cluster_id_str]
            color = final_color_map.get(cluster_id_str, '#888888')
            fig.add_trace(go.Scatter(
                ...
                customdata=cluster_data[["cluster_id_str", "row_idx", "plot_id", "start_hour_float", "day_int", "clip_count", "file_name", "clip_time", "channel", "date_time_str"]].values,
    ...
    def update_spectrogram_plot_from_cache(data, colormap, db_floor):
        ...
        # Ensure audio_path exists in data
        audio_path = data.get('audio_path', '')
        info = data.get('info', 'No data')
        ...
        return info, audio_path, fig, str(data['_rev'])
    ...
    def show_histogram_for_clicked_cluster(clickData, active_cell, hist_type, time_scale, cluster_colors, filtered_data_cache_key, table_data):
        # Determine cluster_id from either scatter plot click or table cell click
        cluster_id = None
        ...
        if ctx.triggered:
            ...
            # Check if triggered by scatter plot click
            if 'scatter.clickData' in trigger_id and clickData:
                cluster_id = str(clickData["points"][0]["customdata"][0])
            
            # Check if triggered by table cell click (any column, not just play_button)
            elif 'data-table.active_cell' in trigger_id and active_cell and table_data:
                # Trigger on any cell click in the table
                row_idx = active_cell.get('row')
                if row_idx is not None and row_idx < len(table_data):
                    # Get cluster_id from the clicked row (convert to string for consistency)
                    cluster_id_val = table_data[row_idx].get('cluster_id', '')
                    if cluster_id_val != '' and cluster_id_val is not None:
                        cluster_id = str(cluster_id_val)
                    else:
                        cluster_id = None
        
        if not cluster_id or not filtered_data_cache_key:
            fig = go.Figure()
            ...
            fig.update_layout(
                ...
                paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                annotations=[
                    {"text": "Click a point or play button to see its histogram", "xref": "paper", "yref": "paper", "showarrow": False,
                     "font": {"size": 14}}]
            )
            return fig
        ...
        # cluster_id is already extracted above (as string)
        # Try to match using cluster_id_str first, then fall back to cluster_id
        if 'cluster_id_str' in dff.columns:
            cluster_df = dff[dff['cluster_id_str'] == cluster_id].copy()
        elif 'cluster_id' in dff.columns:
            # Convert cluster_id to string for comparison
        ...
        if time_scale == 'daily':
            ...
            # Create all_bins_df for consistent binning
            ...
            # Add temperature visualization if temperature column exists
            if 'temperature' in cluster_df.columns:
                # Calculate temperature stats per bin - only for bins that have clips
                temp_stats = cluster_df.groupby('bin')['temperature'].agg(['mean', 'min', 'max']).reset_index()
                
                # Sort by bin - no need to merge with all_bins_df, only show temp where clips exist
                temp_stats = temp_stats.sort_values('bin')
                
                if not temp_stats.empty:
                    # Generate colors based on histogram color
                    dark_color = create_dark_color(bar_color)
                    pale_color = create_pale_color(bar_color, alpha=0.5)
                    ...
                    for i, row in temp_stats.iterrows():
                        if i > 0:
                            prev_bin = temp_stats.iloc[i-1]['bin']
                            curr_bin = row['bin']
                            # If bins are not consecutive, insert NaN to break the line
                            if curr_bin - prev_bin > bin_width:
                                x_line.append(None)
                                y_mean_line.append(None)
                                y_min_line.append(None)
                                y_max_line.append(None)
                        
                        x_line.append(row['bin'])
                        y_mean_line.append(row['mean'])
                        y_min_line.append(row['min'])
                        y_max_line.append(row['max'])
                    
                    # Construct polygon coordinates for band: go forward along max, then backward along min
                    # Filter out None values for the polygon (it will naturally break at gaps)
                    x_forward = [x for x in x_line if x is not None]
                    y_max_forward = [y for y in y_max_line if y is not None]
                    y_min_backward = [y for y in reversed(y_min_line) if y is not None]
                    
                    x_coords = x_forward + list(reversed(x_forward))
                    y_coords = y_max_forward + y_min_backward
                    
                    # Add temperature band (min-max range) - overlay on histogram
                    fig.add_trace(go.Scatter(
                        x=x_coords,
                        y=y_coords,
                        fill='toself',
                    ...
                    # Add average temperature line - overlay on histogram (with breaks at gaps)
                    fig.add_trace(go.Scatter(
                        x=x_line,
                        y=y_mean_line,
                        mode='lines',
                        ...
                        yaxis='y2',
                        hovertemplate='Time: %{x}<br>Avg Temp: %{y:.1f}°C<extra></extra>',
                        connectgaps=False
                    ))
            ...
            fig.update_layout(xaxis=dict(title="Time of Day", range=[0, 1440], tickvals=np.arange(0, 1440, 240),
                                         ticktext=[f"{h // 60:02d}:00" for h in np.arange(0, 1441, 240)]),
                              margin=dict(l=0, r=40 if 'temperature' in cluster_df.columns else 0, t=20, b=40 if 'temperature' in cluster_df.columns else 10), 
                              paper_bgcolor='rgba(0,0,0,0)',
                              plot_bgcolor='rgba(0,0,0,0)')
            if 'temperature' not in cluster_df.columns:
            ...
            # Add temperature visualization if temperature column exists
            if 'temperature' in cluster_df.columns:
                # Calculate temperature stats per week - only for weeks that have clips
                temp_stats = cluster_df.groupby('week_of_year')['temperature'].agg(['mean', 'min', 'max']).reset_index()
                
                # Sort by week - no need to merge with all_bins_df, only show temp where clips exist
                temp_stats = temp_stats.sort_values('week_of_year')
                ...
                if not temp_stats.empty:
                    ...
                    # Configure secondary y-axis for temperature first
                    fig.update_layout(
                        yaxis2=dict(
                            title="Temp (°C)",
                            overlaying='y',
                    ...
                    # Break lines at gaps - insert None between non-consecutive weeks
                    x_line = []
                    y_mean_line = []
                    y_min_line = []
                    y_max_line = []
                    ...
                    for i, row in temp_stats.iterrows():
                        if i > 0:
                            prev_week = temp_stats.iloc[i-1]['week_of_year']
                            curr_week = row['week_of_year']
                            # If weeks are not consecutive, insert None to break the line
                        ...
                        x_line.append(row['week_of_year'])
                        y_mean_line.append(row['mean'])
                        y_min_line.append(row['min'])
                        y_max_line.append(row['max'])
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        line=dict(color=dark_color, width=2.5),
                        yaxis='y2',
                        hovertemplate='Week: %{x}<br>Avg Temp: %{y:.1f}°C<extra></extra>',
                        connectgaps=False
                    ))
